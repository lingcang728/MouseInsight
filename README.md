...
- **绝对紧急自救（EmergencyStop）。** 任何时候按键盘物理急停键——Windows 上是 `Pause` 或 `Scroll Lock`，macOS 上是 `F13` 或 `⌃⌥⌘P`——Mouse Insight 会**无条件立即释放所有按下的虚拟键**，并无条件置为暂停状态（即使此前已经是暂停状态也会执行无条件释放）。若 Windows 急停热键被其他程序占用，应用内会提示，可从托盘暂停。恢复映射只能由用户通过界面「恢复映射」或系统托盘菜单显式恢复，安全键不再承担切换恢复的职责，彻底杜绝卡键。
...
**按键卡死或误操作了怎么自救？**  
随时按下键盘物理急停键——Windows 上是 **`Pause`** 或 **`Scroll Lock`**，macOS 上是 **`F13`** 或 **`⌃⌥⌘P`**。Mouse Insight 会**无条件立即释放所有按下的虚拟键**，并无条件置为暂停状态（即使此前已经是暂停状态也会执行无条件释放）。若 Windows 急停热键被其他程序占用，应用内会提示，可从托盘暂停。恢复映射只能由用户通过界面「恢复映射」或系统托盘菜单显式恢复，安全键不再承担切换恢复的职责，彻底杜绝卡键。
