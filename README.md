<div align="center">
  <img src="src-tauri/icons/icon.png" width="96" height="96" alt="Mouse Insight">
  <h1>Mouse Insight</h1>
...
- **先看见，再绑定。** 点一下「识别鼠标键」，按下鼠标上的任意按键，界面立刻高亮标出系统收到的是哪一颗，直接给它指定键盘快捷键。
- **左键右键永远放行。** 这是底线设计——左键与右键可以检测、可以在界面点亮指示，但**绝对不会被程序拦截或改写**，无论如何都不会把你的操作系统误操作锁死。
- **三种规范触发模式。** 支持「跟随按住」（按下立刻注入、松开立刻释放；点一下也会发短暂脉冲，不是长按阈值）、「单次触发」（回车/截图/独立动作）与「切换保持」（按一次保持、再按一次释放，UI 实时指示当前状态）。侧键与中键另有「短按 / 长按」双槽模式，短按与长按可以各绑一套键。
...
- **绝对紧急自救（EmergencyStop）。** 任何时候按键盘物理急停键——Windows 上是 `Pause`，macOS 上是 `F13` 或 `⌃⌥⌘P`——Mouse Insight 会**无条件立即释放所有按下的虚拟键**，并无条件置为暂停状态（即使此前已经是暂停状态也会执行无条件释放）。Scroll Lock 不再是急停键，不会与 Excel 等程序的正常用法冲突；即使 `Pause` 全局热键被其他程序占用，键盘钩子内仍会兜底识别。恢复映射只能由用户通过界面「恢复映射」或系统托盘菜单显式恢复，安全键不再承担切换恢复的职责，彻底杜绝卡键。
- **窗口关闭即释放内存。** 关闭主界面后会销毁控制面板，后台常驻系统托盘的纯 Rust 核心内存占用低（未做严格量化）。
...
| :--- | :---: | :--- | :--- |
| **后侧键 (XButton1)** | ✅ 支持 | 短按·长按 / 切换保持 | 匹配规则时拦截原鼠标事件，注入设定键盘快捷键。短按·长按的两槽结构覆盖了跟随按住（只填长按）与单次触发（只填短按）。 |
| **前侧键 (XButton2)** | ✅ 支持 | 短按·长按 / 切换保持 | 匹配规则时拦截原鼠标事件，注入设定键盘快捷键。短按·长按的两槽结构覆盖了跟随按住（只填长按）与单次触发（只填短按）。 |
| **鼠标中键 (Middle)** | ✅ 支持 | 短按·长按 / 切换保持 | 匹配规则时拦截原鼠标事件，注入设定键盘快捷键。短按·长按的两槽结构覆盖了跟随按住（只填长按）与单次触发（只填短按）。 |
...
#### 方式一：绿色便携版（推荐）
1. 下载 `Mouse.Insight_<版本>_portable.zip` 解压即用（含 `data/.portable` 标记，配置存于同目录 `data/`）；
2. 若只下载单个 `Mouse.Insight.exe` 直接运行，配置存于 `%APPDATA%\MouseInsight`；旧版同级 `.portable` 与 `config.json` 布局仍兼容；
3. 支持完整的开机自启（在界面勾选「开机自启」后，开机静默常驻托盘，不弹窗打扰）。
...
#### 卸载
安装版卸载会清理自启注册项与日志目录；配置文件 `%APPDATA%\MouseInsight\config.json` 需手动删除。macOS 需手动删除 `~/Library/LaunchAgents/Mouse Insight.plist` 与 `~/Library/Application Support/MouseInsight`。
...
macOS 点击菜单栏的鼠标图标可直接查看监听与映射状态、暂停/恢复映射、打开按键工作台、配置目录、辅助功能设置或版本下载页。图标和下拉菜单采用系统原生绘制，跟随系统深浅色和菜单高亮，不额外创建网页窗口，也不轮询状态。

- 托盘/菜单栏可直接给前侧键、后侧键、中键绑定短按或长按预设，给滚轮绑定滚动操作预设（复制、粘贴、撤销、回车）或清除绑定；菜单显示当前组合，保存后主窗口同步。滚轮没有可按住的状态，因此不提供长按。切换保持模式需在工作台编辑，菜单不会覆盖。最多 5 个鼠标按键映射，每个组合键最多 16 键。
- `⌘,` 打开按键工作台，`⌘⇧P` 暂停/恢复映射，`⌘Q` 释放注入按键后退出。这些是应用菜单快捷键，不会注册为全局热键。
- 关闭控制面板会销毁窗口并释放 WebView；后台映射继续运行。可从菜单栏或 Dock 重新打开；`⌘H` 隐藏后也可恢复。
- 授权失败时菜单显示「监听不可用」，可直接进入辅助功能设置。授权后在控制面板点击「重试监听」即可恢复，无需重启应用。
- Windows 右键托盘提供相同的状态、快捷预设与入口（不含 macOS 辅助功能设置），左键打开控制面板。
...
当前实现使用登录会话级 `CGEventTap` 可修改事件点，首先检查 **辅助功能** 权限。若已授权但仍无法创建监听，检查输入监控及其他鼠标工具；更新应用后，旧权限条目可能需要移除并重新添加当前安装的应用。Windows 不使用这套 macOS 授权流程，监听失败时会显示 Windows 系统错误。
...
1. **退出冲突软件**：与 X-Mouse Button Control 同时运行时两者会互相吞事件，建议只保留一个；
...
* 工作台上方的按键标签可直接切换已配置卡片；带「＋」的标签用于添加映射。卡片支持折叠、拖动顶部甩出切换，也可使用上一张 / 下一张。
* 侧键与中键支持「短按 / 长按」和「切换保持」：只填写短按时单次触发，只填写长按时按下立刻跟随；两套都填时用长按阈值区分。滚轮只支持单次触发（滚动操作）。
* 编辑后自动保存，界面显示保存结果。浏览、折叠和切换卡片不会写入配置；普通文案不会因双击而选中或保存，配置路径仍可复制。
* 录制可以取消；识别在 15 秒后自动结束，切换到其他窗口也会结束录制与识别。
...
| **短按 / 长按 (Dual)** | 短按与长按各绑一套键：按住不超过阈值触发短按槽，超过 0.4 秒切换为长按槽。只填短按 = 每按一下触发一次；只填长按 = 按住期间持续生效。 | **一键两用**。例如短按回车、长按呼出语音输入。 | 侧键前/后、中键支持。 |
...
> [!NOTE]
> **关于单次触发与驻留时间的标准规范表述：**  
> 单次触发使用一个短暂的可控驻留时间，以提高部分全局热键应用对模拟按键的兼容性。驻留时间属于兼容性策略，应由实测确定。Microsoft SendInput 保证输入事件按顺序插入，单次触发通过非阻塞调度避免阻塞其他按键。每个按键最多排队 32 次单次触发，超出部分不再积压；原生输入事件队列溢出时自动暂停并释放已保持的键，请从界面恢复。
...
**按键卡死或误操作了怎么自救？**  
随时按下键盘物理急停键——Windows 上是 **`Pause`**，macOS 上是 **`F13`** 或 **`⌃⌥⌘P`**。Mouse Insight 会**无条件立即释放所有按下的虚拟键**，并无条件置为暂停状态（即使此前已经是暂停状态也会执行无条件释放）。即使 `Pause` 全局热键被其他程序占用，键盘钩子内仍会兜底识别；也可以从托盘菜单暂停。恢复映射只能由用户通过界面「恢复映射」或系统托盘菜单显式恢复，安全键不再承担切换恢复的职责，彻底杜绝卡键。

**程序占用资源大吗？**  
关主窗口会销毁控制面板（Windows 上是 WebView2），后台只留钩子引擎；点托盘会重建控制面板。若托盘单击无响应，用右键菜单「打开按键工作台…」。
...
**macOS 上侧键没反应？**  
先确认 **系统设置 → 隐私与安全性 → 辅助功能** 里已经打开 Mouse Insight。该权限被关掉时，全局事件点创建会失败，控制面板会显示授权提示。授权后点击「重试监听」即可恢复，无需重启。系统临时禁用事件点时，应用会暂停映射并释放按键；重新授权后点击「重试监听」重建监听，再手动恢复映射。

**强杀进程后感觉有键被"按住"？**  
正常退出（含安装/卸载前的 `--quit` 优雅退出）会先释放所有注入按键；但任务管理器强杀、`taskkill /F`、断电等进程没机会执行清理的路径，Windows 不会自动复位注入键态。若疑似卡住，物理按放一次对应的修饰键（Ctrl/Alt/Shift/Win）即可恢复；下次启动应用也会自动补发遗留的 key-up。
...
日志位于 `<配置目录>/logs/`（Windows 安装版为 `%APPDATA%\MouseInsight\logs\`），可从托盘菜单「打开日志目录…」直接进入；发生 panic 时会额外写入 `crash-*.log`。日志只记录运行状态与错误，**不记录任何按键内容**。
...
打包脚本先验证新程序与安装包，再替换本地 `release/`，不会创建 Tag 或发布 GitHub Release。运行中的便携版请先从托盘退出；自动替换当前实例可使用 `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/package-release.ps1 -StopRunningApp`。原配置按原字节迁移到 `release/data/`，旧文件备份到 `%LOCALAPPDATA%\MouseInsight\package-backups/`。`release/publish/` 内为已改好点号文件名的待上传产物（便携 exe、安装包、便携 zip）与 `SHA256SUMS.txt`。

> [!NOTE]
> **关于 beta 版本发布**：发布 beta 版本时不要勾选 GitHub 的 pre-release，否则应用内检查更新会跳过它。
