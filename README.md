...
* 点击映射卡片上的模式下拉框，可以随时在「跟随按住」、「单次触发」、「切换保持」之间切换（录入按键时系统会自动推断推荐模式：纯修饰键推荐跟随按住，包含普通按键推荐单次触发）；
...
> [!NOTE]
> **关于单次触发与驻留时间的标准规范表述：**  
> 单次触发使用一个短暂的可控驻留时间，以提高部分全局热键应用对模拟按键的兼容性。驻留时间属于兼容性策略，应由实测确定。Microsoft SendInput 保证输入事件按顺序插入，单次触发内部通过非阻塞调度保证高频触发下事件不丢失也不阻塞其他按键。
